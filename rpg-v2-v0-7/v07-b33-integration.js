(function(){'use strict';
function patch(src){
 const must=(from,to,label)=>{if(!src.includes(from))throw new Error('B34 '+label+' anchor missing');src=src.replace(from,to)};
...
function v07Near(a,b){return Math.abs(a.r-b.r)<=1&&Math.abs(a.c-b.c)<=1&&(a.r!==b.r||a.c!==b.c)}
function v07FindChargeTarget(){let b=v07BossUnit();if(!b)return null;let out=[];for(let h of livingHeroes().filter(h=>h.r===b.r||h.c===b.c)){let dr=Math.sign(h.r-b.r),dc=Math.sign(h.c-b.c),dist=Math.abs(h.r-b.r)+Math.abs(h.c-b.c),clear=true;for(let k=1;k<dist;k++)if(v07At(b.r+dr*k,b.c+dc*k,b.id)){clear=false;break}if(clear)out.push({h,dr,dc,dist})}return out.sort((a,z)=>a.dist-z.dist||a.h.id-z.h.id)[0]||null}
async function v07Charge(t){let b=v07BossUnit();if(!b||!t)return;let from={r:b.r,c:b.c},steps=Math.min(2,Math.max(0,t.dist-1)),to={r:b.r+t.dr*steps,c:b.c+t.dc*steps},pushed=[];b.r=to.r;b.c=to.c;if(rem(t.h)>0&&Math.abs(t.h.r-b.r)+Math.abs(t.h.c-b.c)===1){let chain=[],r=t.h.r,c=t.h.c;while(true){let u=v07At(r,c,b.id);if(!u)break;chain.push(u);r+=t.dr;c+=t.dc}for(let i=chain.length-1;i>=0;i--){let u=chain[i],fr={r:u.r,c:u.c},nr=u.r+t.dr,nc=u.c+t.dc,to2={r:nr,c:nc};pushed.push({unit:u,from:fr,to:to2});if(nr<0||nr>=ROWS||nc<0||nc>=COLS){u.hits=max(u);u.noReviveFromCollapse=true}else{u.r=nr;u.c=nc}}let ev=[];if(rem(t.h)>0)hitHero(t.h,1,b,ev);if(v07BossAnim)await v07BossAnim.charge(b,from,to,t.h,pushed);if(ev.length)await settle(ev);else render()}else{if(v07BossAnim)await v07BossAnim.charge(b,from,to,null,[]);render()}}
async function v07Quake(){let b=v07BossUnit();if(!b)return;let ts=livingHeroes().filter(h=>v07Near(b,h));if(v07BossAnim)await v07BossAnim.quake(b,ts);let ev=[];for(let h of ts)hitHero(h,2,b,ev);if(ev.length)await settle(ev);else render()}
...
function v07CreateBossEngine(b){v07BossUnitId=b.id;v07WarningFresh=false;v07BossEngine=new GiantElephantGuard({bossAlive:()=>!!v07BossUnit(),findChargeTarget:v07FindChargeTarget,hasAdjacentHero:()=>{let x=v07BossUnit();return !!x&&livingHeroes().some(h=>v07Near(x,h))},charge:v07Charge,quake:v07Quake,stomp:v07Stomp,notice:v07Notice,getRows:()=>ROWS,setWarningRow:r=>{bossWarningRow=r;rebuildGrid()},render,collapseBottomRow:v07Collapse});v07BossEngine.name='巨像守衛';v07BossEngine.activate();v07BossEngine.stopDefeatWatch()}`;
 ...
 const baseTexts=[
 ["特殊戰士接觸敵人時，對接觸的敵人造成 1 點傷害。","特殊戰士進場時，若周圍 8 格內有敵人，立即對其中接觸的敵人造成 1 點傷害。"],
 ["特殊戰士實際移動後接觸敵人時發動近戰攻擊。","特殊戰士必須在一次有效滑動中實際改變格位；移動結束後若周圍 8 格內接觸敵人，立即發動近戰攻擊。未實際移動不觸發。"],
 ["特殊遊俠實際移動後，有 50% 機率對周圍 8 格以外最遠的敵人造成 1 點傷害。","特殊遊俠必須在一次有效滑動中實際改變格位；移動結束後有 35% 機率發動遠射，從遊俠周圍 8 格以外的敵人中選擇距離最遠的 1 名造成 1 點傷害。周圍 8 格內的敵人不是遠射目標；未實際移動不觸發。"],
 ["遊俠移動後，向本次移動的反方向射擊；該方向有敵人時攻擊第一個敵人。","特殊遊俠必須實際移動；移動結束後沿本次移動的反方向直線射擊，若該直線上有敵人，攻擊距離遊俠最近的第 1 名敵人。未實際移動或反方向直線沒有敵人時不觸發。"],
 ["特殊祭司進場時，治療周圍 8 格內的我方單位 1 HP。","特殊祭司每次實際進場時觸發治癒波，治療自己周圍 8 格內仍存活的友方單位 1 HP；不包含祭司自己。周圍沒有受傷友方時仍視為已觸發。"],
 ["祭司實際移動後，從自己周圍 8 格內的友方單位中隨機選擇最多 3 名獲得守護，不包含祭司自己；優先選擇目前沒有守護的友方。下一次受到傷害時傷害 -1，之後消耗。","特殊祭司必須在一次有效滑動中實際改變格位；移動結束後，從自己周圍 8 格內仍存活的友方中隨機選擇最多 3 名獲得守護，不包含祭司自己，並優先選擇目前沒有守護的友方。守護使下一次受到的傷害 -1，之後消耗；未實際移動不觸發。"]
 ];
 for(const [a,b] of baseTexts)if(src.includes(a))src=src.replaceAll(a,b);
