...
async function v07Quake(){let b=v07BossUnit();if(!b)return;let ts=livingHeroes().filter(h=>v07Near(b,h));if(v07BossAnim)await v07BossAnim.quake(b,ts);let ev=[];for(let h of ts)hitHero(h,1,b,ev);if(ev.length)await settle(ev);else render()}
