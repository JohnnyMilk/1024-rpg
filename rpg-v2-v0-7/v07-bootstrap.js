...
const patch=`
// v0.6 final rules inherited by v0.7. v0.6 files remain frozen.
...
// Priest Guard: self + up to 3 nearby allies; prioritize allies without Guard.
...
// v0.7 rectangular battlefield foundation: 4 columns, dynamic 4/5/6 rows.
src=src.replace("const N=4,BI=20,MM=TIMING.MOVE,GM=TIMING.MERGE,FX=TIMING.EFFECT,DM=TIMING.DEATH;","let ROWS=4;const COLS=4,BI=20,MM=TIMING.MOVE,GM=TIMING.MERGE,FX=TIMING.EFFECT,DM=TIMING.DEATH;");
src=src.replace("for(let i=0;i<N*N;i++){let d=document.createElement('div');d.className='cell';grid.appendChild(d)}","let bossCollapseStage=0,bossWarningRow=-1;function rebuildGrid(){grid.innerHTML='';board.style.setProperty('--v07-rows',ROWS);board.classList.toggle('v07-expanded',ROWS>4);for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++){let d=document.createElement('div');d.className='cell'+(r===bossWarningRow?' v07-warning':'');d.dataset.r=r;d.dataset.c=c;grid.appendChild(d)}}rebuildGrid();");
src=src.replace("function empty(){return emptyCells(E,N)}","function empty(){let a=[];for(let r=0;r<ROWS;r++)for(let c=0;c<COLS;c++)if(!E.some(e=>rem(e)>0&&e.r===r&&e.c===c))a.push([r,c]);return a}");
src=src.replace("for(let k=1;k<N;k++){let r=source.r+dr*k,c=source.c+dc*k;if(r<0||r>=N||c<0||c>=N)break;","for(let k=1;k<Math.max(ROWS,COLS);k++){let r=source.r+dr*k,c=source.c+dc*k;if(r<0||r>=ROWS||c<0||c>=COLS)break;");
src=src.replaceAll("nr<0||nr>=N||nc<0||nc>=N","nr<0||nr>=ROWS||nc<0||nc>=COLS");

// Boss enters from the new top area. Existing units are pushed down two rows.
const oldBossSpawn="if(turn>=next&&!bactive){let b=addEnemy(true);if(b){bactive=true;render();spawnFx(b);await wait(TIMING.SPAWN);let be=[];if(has('firstStrike'))hitEnemy(b,2,be,'firstStrike','⚡');await settle(be)}}";
const newBossSpawn="if(turn>=next&&!bactive){ROWS=6;bossCollapseStage=0;bossWarningRow=-1;for(let u of E)u.r+=2;rebuildGrid();render();await wait(420);let p=randomItem(empty().filter(x=>x[0]<2))||randomItem(empty());if(p){let b={id:id++,type:'boss',r:p[0],c:p[1],hits:0,maxHits:bmax(),damage:bdmg()};E.push(b);bactive=true;render();spawnFx(b);anim.play('skill-tag',{text:'👑 巨象守門者 · 戰場擴張 4×6'});await wait(TIMING.SPAWN);let be=[];if(has('firstStrike'))hitEnemy(b,2,be,'firstStrike','⚡');await settle(be)}}";
if(!src.includes(oldBossSpawn))throw new Error('找不到 Boss 生成片段');
src=src.replace(oldBossSpawn,newBossSpawn);

// Boss death starts two-step warned collapse instead of snapping back immediately.
src=src.replace("bactive=false;next=turn+BI;bossKills++","bactive=false;next=turn+BI;bossKills++;bossCollapseStage=1;bossWarningRow=ROWS-1;rebuildGrid();anim.play('skill-tag',{text:'⚠️ 戰場崩塌預警 · 裂痕格仍可使用'})");
const collapseFn=\`async function v07CollapseStep(){if(!bossCollapseStage)return;let row=bossWarningRow,cells=[...grid.children].filter(x=>+x.dataset.r===row);cells.forEach(x=>x.classList.add('v07-collapsing'));anim.play('skill-tag',{text:'⬇️ ENEMY PHASE · 警告格崩塌'});await wait(700);let doomed=E.filter(x=>x.r===row&&rem(x)>0);for(let e of doomed){e.hits=max(e);e.noReviveFromCollapse=true}if(doomed.length)await settle([]);E=E.filter(x=>x.r!==row);ROWS--;bossWarningRow=-1;if(ROWS>4){bossCollapseStage++;bossWarningRow=ROWS-1;rebuildGrid();render();anim.play('skill-tag',{text:'⚠️ 下一排龜裂 · 還有一次 Player Phase'})}else{bossCollapseStage=0;rebuildGrid();render();anim.play('skill-tag',{text:'✓ 戰場恢復 4×4'})}}\`;
src=src.replace("function tryResurrect(hero){if(!hero||hero.type!=='hero'||hero.revived||!hasClass('priestRevive'))return null;","function tryResurrect(hero){if(!hero||hero.noReviveFromCollapse||hero.type!=='hero'||hero.revived||!hasClass('priestRevive'))return null;");
src=src.replace("async function move(d){",collapseFn+"\\nasync function move(d){");
src=src.replace("turn++;render();await enemyPhase();if(!livingHeroes().length){check();busy=false;return}await postTurn();","turn++;render();if(bossCollapseStage){await v07CollapseStep()}else{await enemyPhase()}if(!livingHeroes().length){check();busy=false;return}await postTurn();");
src=src.replace("function reset(){E=[];turn=0;","function reset(){ROWS=4;bossCollapseStage=0;bossWarningRow=-1;rebuildGrid();E=[];turn=0;");
`;
