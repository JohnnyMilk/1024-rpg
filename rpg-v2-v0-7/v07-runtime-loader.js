...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  /* B29: mirror REUSE-06 state transition: WARNING -> PLAYER_WARNING -> COLLAPSING. */
  const collapseInjectTarget='src=src.replace("async function move(d){",collapseFn+"\\\\nasync function move(d){");';
  const lifecycle="let v07MapState='IDLE';async function v07BeginCollapse(){if(!window.__v07CollapsePending||ROWS<=4)return;window.__v07CollapsePending=false;bossCollapseStage=1;bossCollapseFresh=false;bossWarningRow=ROWS-1;v07MapState='WARNING';let tag=anim.play('skill-tag',{text:'⚠️ BOSS DEFEATED · 戰場即將崩落',duration:3000});if(tag)tag.style.animationDuration='3000ms';await wait(3000);rebuildGrid();render()}function v07AfterPlayerPhase(){if(v07MapState==='WARNING')v07MapState='PLAYER_WARNING'}async function v07MapEventPhase(){if(v07MapState!=='PLAYER_WARNING'||!bossCollapseStage||bossWarningRow<0)return;v07MapState='COLLAPSING';await v07CollapseStep();if(ROWS>4&&bossCollapseStage){v07MapState='WARNING'}else{v07MapState='DONE'}}";
  const collapseInjectReplacement='src=src.replace("async function move(d){",collapseFn+"\\\\n'+lifecycle.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'\\\\nasync function move(d){");';
  if(!code.includes(collapseInjectTarget))throw new Error('B29 找不到 game runtime lifecycle 注入點');
  ...
  const newPhasePatch='src=src.replace("turn++;render();await enemyPhase();if(!livingHeroes().length){check();busy=false;return}await postTurn();","turn++;render();v07AfterPlayerPhase();await enemyPhase();if(!livingHeroes().length){check();busy=false;return}await v07MapEventPhase();if(!livingHeroes().length){check();busy=false;return}await postTurn();");';
  if(!code.includes(oldPhasePatch))throw new Error('B29 找不到 Enemy/Map Event phase 注入點');
  ...
  if(!code.includes(settleAnchor))throw new Error('B29 找不到 Boss reward settlement 注入點');
  code=code.replace(settleAnchor,rewardHook+'\n'+settleAnchor);

  (0,eval)(code+'\n//# sourceURL=rpg-v2-v0-7/v07-bootstrap-fixed-b29.js');
