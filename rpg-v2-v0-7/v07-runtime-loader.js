...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  // Add the collapse gate directly at the beginning of the v0.7 patch template.
  // This avoids matching another source-patch statement such as tryResurrect, which is intentionally rewritten inside the same template.
  const patchStart="const patch=`\n";
  const enemyPatch="src=src.replace(\"async function enemyPhase(){let p=enemyPlan();\",\"async function enemyPhase(){if(bossCollapseStage){if(bossCollapseFresh){bossCollapseFresh=false}else{return await v07CollapseStep()}}let p=enemyPlan();\");\n";
