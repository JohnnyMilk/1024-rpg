...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  // Give the Boss warning enough time to read before the board actually expands.
  ...
  // Keep collapse warning on screen long enough to read.
  ...
  // Make units occupying a collapsing row fall together with the cells.
  ...
  // The previous move-body hook was too brittle after the frozen v0.6 transforms.
  // Hook enemyPhase itself: Boss death gets one full warning Player Phase, then the next Enemy Phase collapses one row.
  const hook="src=src.replace(\"function tryResurrect(hero){if(!hero||hero.noReviveFromCollapse";
  ...
  if(!code.includes(hook))throw new Error('找不到 v0.7 Enemy Phase 注入點');
