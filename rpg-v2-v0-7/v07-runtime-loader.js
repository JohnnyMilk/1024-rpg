...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  /* REUSE-06 collapse animation: staggered floor fall + units fall with the row. */
  ...
  /* B25: actual game runtime. REUSE-06 semantics are WARNING -> one real Player Phase -> that move's Enemy Phase collapses. */
  const collapseInjectTarget='src=src.replace("async function move(d){",collapseFn+"\\\\nasync function move(d){");';
  const collapseLifecycle="async function v07BeginCollapse(){if(!window.__v07CollapsePending||ROWS<=4)return;window.__v07CollapsePending=false;bossCollapseStage=1;bossCollapseFresh=false;bossWarningRow=ROWS-1;let tag=anim.play('skill-tag',{text:'⚠️ BOSS DEFEATED · 戰場即將崩落',duration:3000});if(tag)tag.style.animationDuration='3000ms';await wait(3000);rebuildGrid();render()}";
  const collapseInjectReplacement='src=src.replace("async function move(d){",collapseFn+"\\\\n'+collapseLifecycle.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'\\\\nasync function move(d){");';
  if(!code.includes(collapseInjectTarget))throw new Error('B25 找不到實際 game runtime lifecycle 注入點');
  ...
  if(!code.includes(settleAnchor))throw new Error('B25 找不到 Boss reward settlement 注入點');
