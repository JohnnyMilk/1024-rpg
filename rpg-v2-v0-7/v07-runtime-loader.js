...
const fail=msg=>{console.error(msg);if(window.__v07Report)window.__v07Report('Loader · '+msg);else if(bootError){bootError.style.display='block';bootError.textContent='v0.7 B35-H8 載入失敗：'+msg}};
fetch('v07-bootstrap.js?v=20260916-b35h8').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  if(!code.includes(oldGuardRule))throw new Error('B35-H8 Priest guard anchor missing');code=code.replace(oldGuardRule,newGuardRule);
  ...
  if(!code.includes(oldGuardText))throw new Error('B35-H8 Priest guard UI anchor missing');code=code.replace(oldGuardText,newGuardText);
  ...
  if(!code.includes(b33Hook))throw new Error('B35-H8 final-runtime hook missing');
  code=code.replace(b33Hook,b33Hook+"\nsrc=window.V07B33.patch(src);");
  (0,eval)(code+'\n//# sourceURL=rpg-v2-v0-7/v07-bootstrap-fixed-b35h8.js');
