...
const fail=msg=>{console.error(msg);if(window.__v07Report)window.__v07Report('Loader · '+msg);else if(bootError){bootError.style.display='block';bootError.textContent='v0.7 B31 載入失敗：'+msg}};
fetch('v07-bootstrap.js?v=20260916-b31').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  // B31: keep the original Boss reward loop, then begin the map-event warning only after the reward choice closes.
  const defeatOld="bossCollapseStage=1;bossCollapseFresh=true;bossWarningRow=ROWS-1;rebuildGrid();anim.play('skill-tag',{text:'⚠️ 戰場崩塌預警 · 裂痕格仍可使用'})";
  const defeatNew="bossCollapseStage=1;bossCollapseFresh=true;bossWarningRow=ROWS-1";
  if(!code.includes(defeatOld))throw new Error('B31 Boss defeat state anchor missing');
  code=code.replace(defeatOld,defeatNew);
  const rewardOld="for(let i=0;i<bossKills;i++)if(profession)await chooseClassSkill();maybeAwaken();return true";
  const rewardNew="for(let i=0;i<bossKills;i++)if(profession)await chooseClassSkill();if(bossKills&&bossCollapseStage===1){let v07DefeatTag=anim.play('skill-tag',{text:'⚠️ BOSS DEFEATED · 戰場即將崩落',duration:3000});if(v07DefeatTag)v07DefeatTag.style.animationDuration='3000ms';await wait(3000);rebuildGrid();render()}maybeAwaken();return true";
  if(!code.includes(rewardOld))throw new Error('B31 Boss reward anchor missing');
  code=code.replace(rewardOld,rewardNew);

  code=code.replace("anim.play('skill-tag',{text:'⚠️ 下一排龜裂 · 還有一次 Player Phase'})","anim.play('skill-tag',{text:'⚠️ 下一排龜裂 · 保留一次 Player Phase'})");
  ...
  if(!code.includes(oldGuardRule))throw new Error('B31 Priest guard anchor missing');code=code.replace(oldGuardRule,newGuardRule);
  ...
  if(!code.includes(oldGuardText))throw new Error('B31 Priest guard UI anchor missing');code=code.replace(oldGuardText,newGuardText);
  (0,eval)(code+'\n//# sourceURL=rpg-v2-v0-7/v07-bootstrap-fixed-b31.js');
