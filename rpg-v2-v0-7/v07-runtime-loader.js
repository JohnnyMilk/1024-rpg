...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  /* B28: animate the exact warning DOM nodes themselves. No row selector reconstruction. */
  code=code.replace("let row=bossWarningRow,cells=[...grid.children].filter(x=>+x.dataset.r===row);cells.forEach(x=>x.classList.add('v07-collapsing'));anim.play('skill-tag',{text:'⬇️ ENEMY PHASE · 警告格崩塌'});await wait(700);let doomed=E.filter(x=>x.r===row&&rem(x)>0);","let row=bossWarningRow,cells=[...grid.querySelectorAll('.v07-warning')],doomed=E.filter(x=>x.r===row&&rem(x)>0),v07s=step();anim.play('skill-tag',{text:'⬇️ MAP EVENT · 警告格正在崩落'});for(let i=0;i<cells.length;i++){let x=cells[i];x.classList.remove('v07-warning');x.classList.add('v07-collapsing');x.style.transformOrigin='50% 100%';x.style.willChange='transform,opacity';let a=x.animate([{opacity:1,transform:'none'},{opacity:.85,transform:'perspective(500px) rotateX(20deg) scale(.92)',offset:.35},{opacity:0,transform:'perspective(500px) translateY(90px) rotateX(72deg) rotateZ(18deg) scale(.18)'}],{duration:760,easing:'ease-in',fill:'forwards'});let u=doomed.find(e=>e.c===+x.dataset.c);if(u){let pe=pieces.querySelector('[data-id=\"'+u.id+'\"]');if(pe&&pe.animate)pe.animate([{opacity:1,transform:pe.style.transform},{opacity:0,transform:'translate('+(u.c*v07s)+'px,'+((u.r+1.2)*v07s)+'px) rotate(18deg) scale(.2)'}],{duration:760,easing:'ease-in',fill:'forwards'})}await wait(120)}await wait(820);");
  ...
  /* B28 three-phase loop. Warning nodes are created once at event start and consumed by the next Map Event. */
  ...
  if(!code.includes(collapseInjectTarget))throw new Error('B28 找不到實際 game runtime lifecycle 注入點');
  ...
  if(!code.includes(oldPhasePatch))throw new Error('B28 找不到舊 Enemy/Collapse phase patch');
  ...
  if(!code.includes(settleAnchor))throw new Error('B28 找不到 Boss reward settlement 注入點');
