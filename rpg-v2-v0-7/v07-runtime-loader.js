...
fetch('v07-bootstrap.js?v=20260915g').then(r=>{if(!r.ok)throw new Error('v07-bootstrap '+r.status);return r.text()}).then(code=>{
  ...
  code=code.replace("src=src.replace(\"bactive=false;next=turn+BI;bossKills++\",\"bactive=false;next=turn+BI;bossKills++;bossCollapseStage=1;bossCollapseFresh=true;bossWarningRow=ROWS-1;rebuildGrid();anim.play('skill-tag',{text:'⚠️ 戰場崩塌預警 · 裂痕格仍可使用'})\");","src=src.replace(\"bactive=false;next=turn+BI;bossKills++\",\"bactive=false;next=turn+BI;bossKills++;bossCollapseStage=1;bossCollapseFresh=true;bossWarningRow=-1;window.__v07CollapsePending=true\");");
  ...
  /* B26: explicit three-phase loop. MAP EVENT always runs after Enemy Phase and is a no-op when no map event is pending. */
  const collapseInjectTarget='src=src.replace("async function move(d){",collapseFn+"\\\\nasync function move(d){");';
  const lifecycle="async function v07BeginCollapse(){if(!window.__v07CollapsePending||ROWS<=4)return;window.__v07CollapsePending=false;bossCollapseStage=1;bossCollapseFresh=true;bossWarningRow=ROWS-1;let tag=anim.play('skill-tag',{text:'⚠️ BOSS DEFEATED · 戰場即將崩落',duration:3000});if(tag)tag.style.animationDuration='3000ms';await wait(3000);rebuildGrid();render()}async function v07MapEventPhase(){if(!bossCollapseStage)return;if(bossCollapseFresh){bossCollapseFresh=false;return}await v07CollapseStep()}";
  const collapseInjectReplacement='src=src.replace("async function move(d){",collapseFn+"\\\\n'+lifecycle.replace(/\\/g,'\\\\').replace(/"/g,'\\"')+'\\\\nasync function move(d){");';
  if(!code.includes(collapseInjectTarget))throw new Error('B26 找不到實際 game runtime lifecycle 注入點');
  ...
  if(!code.includes(oldPhasePatch))throw new Error('B26 找不到舊 Enemy/Collapse phase patch');
  ...
  if(!code.includes(settleAnchor))throw new Error('B26 找不到 Boss reward settlement 注入點');
