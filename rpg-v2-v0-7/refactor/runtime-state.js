...
class V07RuntimeState{
 ...
 reset(config={}){this.config={...config};this.rows=config.rows||4;this.cols=config.cols||4;this.turn=0;this.phase='IDLE';this.units=[];this.warningRow=-1;this.activeBoss=null;this.ended=false;this.busy=false;return this}
 ...
 snapshot(){return{mode:this.config.mode,rows:this.rows,cols:this.cols,turn:this.turn,phase:this.phase,warningRow:this.warningRow,boss:this.activeBoss?.id||null,units:this.units.length}}
}
