...
const V07UnitRules={
 ...
 mergeInto(target,source){if(!this.canMerge(target,source))return false;const hp=this.remainingHP(target)+this.remainingHP(source),nextTier=target.tier*2,nextMax=this.heroMax(nextTier);target.tier=nextTier;target.hits=Math.max(0,nextMax-Math.min(nextMax,hp));source.dead=true;return target}
};
