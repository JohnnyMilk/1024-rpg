...
class V07MapEventManager{
 ...
 async run(){if(!this.active)this.active=this.queue.shift()||null;if(!this.active)return false;const e=this.active;if(e.type==='expand')return this.runExpand(e);if(e.type==='collapse')return this.runCollapse(e);throw new Error('Unknown map event: '+e.type)}
 async runExpand(e){if(e.state!=='QUEUED')return false;const from=this.a.rows(),to=Math.max(from,e.toRows||from);e.state='EXPANDING';this.trace('[V07 MAP] EXPANDING',{from,to});await this.a.expandRows?.(to-from,e);e.state='DONE';this.active=null;this.a.render();this.trace('[V07 MAP] EXPANDED',{rows:this.a.rows()});return true}
