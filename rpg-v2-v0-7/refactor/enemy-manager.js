...
class V07EnemyManager{
 ...
 async attack(enemy,hero){let damage=enemy.damage||1;if(hero.guard){hero.guard=false;delete hero.guardBy;damage=0;await this.r.events.emit('GUARD_BLOCK',{hero,enemy})}if(damage>0)await this.r.damage.apply(hero,damage,{source:enemy,kind:'enemy-contact'});await this.r.events.emit('ENEMY_ATTACK',{enemy,hero,damage});return true}
}
