...
class V07ProfessionManager{
 ...
 async onMove(event){if(!this.profession)return;const moves=event.transaction.actualMoves||event.transaction.moved;for(const m of moves){let h=this.r.state.units.find(x=>x.id===m.id);if(!h&&m.mergedInto)h=this.r.state.units.find(x=>x.id===m.mergedInto);if(!h||!h.special||h.profession!==this.profession)continue;if(this.profession==='priest')await this.priestGuard(h);await this.r.events.emit('PROFESSION_MOVE',{profession:this.profession,hero:h,movement:m,transaction:event.transaction})}}
