...
class V07ProfessionManager{
 ...
 async priestGuard(priest){const heroes=this.r.state.living('hero'),v07=this.r.config.features?.priestGuardV07!==false;let targets;if(v07)targets=V07PriestRules.chooseGuardTargets(priest,heroes,(list,n)=>this.randomTargets(list,n));else targets=heroes.filter(x=>Math.abs(x.r-priest.r)<=1&&Math.abs(x.c-priest.c)<=1);for(const h of targets){h.guard=true;h.guardBy=priest.id}if(targets.length){this.trace('PRIEST_GUARD',{priest:priest.id,targets:targets.map(x=>x.id)});await this.r.events.emit('PRIEST_GUARD',{priest,targets})}return targets}
