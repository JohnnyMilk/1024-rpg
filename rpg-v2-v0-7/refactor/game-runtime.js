...
class V07GameRuntime{
 constructor({mode='normal',config={},adapter,trace}={}){
  ...
  this.trace=trace||((event,data)=>console.log('[V07]',event,data||''));
  this.config=V07GameConfig.get(mode,config);this.state=new V07RuntimeState(this.config);this.adapter=adapter;
  this.mapEvents=new V07MapEventManager({rows:()=>this.state.rows,setWarningRow:r=>{this.state.setWarningRow(r);this.adapter.setWarningRow?.(r)},notice:(...a)=>this.adapter.notice?.(...a),render:()=>this.adapter.render?.(this.state),collapseBottomRow:async()=>{await this.adapter.collapseBottomRow?.(this.state);this.state.setRows(this.state.rows-1)}},(...a)=>this.trace('MAP',a));
 ...
 async boot(){this.trace('BOOT',this.state.snapshot());await this.adapter.boot?.(this.state,this.config);if(this.config.boss)await this.spawnBoss(this.config.boss);this.adapter.render?.(this.state);return this}
 bossContext(){return{game:this.adapter.bossGameAdapter?.(this.state)||this.adapter,mapEvents:this.mapEvents,animations:this.adapter.animations,trace:(e,d)=>this.trace(e,d)}}
 ...
 async playerPhase(action){this.state.setPhase('PLAYER');const result=await this.adapter.playerPhase?.(action,this.state);if(result===false)return false;this.state.nextTurn();return result}
 async enemyPhase(){this.state.setPhase('ENEMY');await this.adapter.enemyPhase?.(this.state);if(this.state.activeBoss?.isAlive())await this.state.activeBoss.takeTurn();return true}
