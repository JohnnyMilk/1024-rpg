...
class V07SkillSystem{
 ...
 rank(id){return this.r.mastery?.rank(id)||'LV1'}
 level(id){return this.r.mastery?.level(id)||1}
 grant(id){if(!this.definitions.has(id))throw new Error('Unknown skill: '+id);if(this.has(id))return false;this.r.state.skills.add(id);this.bindOne(this.definitions.get(id));this.trace('SKILL_GRANTED',{id,rank:this.rank(id),level:this.level(id)});return true}
