...
class V07EnemyManager{
 ...
 async run(){const phaseEnemies=[...this.r.state.living('enemy')];if(this.r.state.skipNextNormalEnemyActions){this.r.state.skipNextNormalEnemyActions=false;this.trace('ENEMY_PHASE_SKIPPED',{count:phaseEnemies.length,source:'swift-max'});return true}for(const enemy of phaseEnemies){if(!this.r.units.alive(enemy))continue;await this.takeTurn(enemy)}return true}
