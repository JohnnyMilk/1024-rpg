(function(){'use strict';
const ICONS={warSweep:'🌀',rangerSnipe:'🔭',priestPunish:'☀️',ascend:'⏫',awakening:'🔱'};
...
function patchVisibleLabels(){document.querySelectorAll('.animSkillTag').forEach(el=>{el.textContent=el.textContent.replace(/^👑 覺醒/,'🔱 覺醒').replace(/^🌟 躍升/,'⏫ 躍升').replace(/^🎯 狙擊/,'🔭 狙擊').replace(/^🗡️ 橫掃/,'🌀 橫掃').replace(/^✨ 懲戒/,'☀️ 懲戒')})}
