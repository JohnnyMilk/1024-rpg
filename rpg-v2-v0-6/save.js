...
function recordBestHero(value){value=Math.max(0,Number(value)||0);if(!value)return;let s=load();if(value>(s.stats.bestHero||0)){s.stats.bestHero=value;save(s);window.dispatchEvent(new CustomEvent('rpg-save-updated',{detail:{type:'bestHero',value}}))}}
function recordRun(r){let s=load(),st=s.stats;st.gamesPlayed++;st.totalMoves+=r.moves||0;st.totalMerges+=r.merges||0;st.totalEnemiesKilled+=r.enemyDefeated||0;st.totalBossKills+=r.bossKills||0;st.totalHeroesLost+=r.heroesLost||0;st.bestHero=Math.max(st.bestHero||0,r.bestHero||0);st.bestScore=Math.max(st.bestScore||0,r.score||0);save(s);window.dispatchEvent(new CustomEvent('rpg-save-updated',{detail:{type:'run'}}))}
...
window.RPGSave={KEY,CURRENT,load,save,uses,rankFromUses,progress,recordSkillUse,recordBestHero,recordRun,clear};
})();