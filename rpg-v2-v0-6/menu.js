...
let historyTab='overview',swipeStart=null;
...
function currentDesc(sk,rank){if(window.RPGMasteryUI&&typeof window.RPGMasteryUI.description==='function')return window.RPGMasteryUI.description(sk.name,rank,sk.desc);return sk.desc}
function skillCard(sk){let u=RPGSave.uses(sk.id),p=RPGSave.progress(u),pct=p.rank==='MAX'?100:Math.round(p.current/p.target*100);return '<article class="historySkill"><div class="hsTop"><span class="hsIcon">'+sk.icon+'</span><div><b>'+sk.name+'</b><em>'+p.rank+'</em></div></div><p class="hsDesc">'+currentDesc(sk,p.rank)+'</p><div class="meter"><i style="width:'+pct+'%"></i></div><div class="hsMeta"><span>'+(p.rank==='MAX'?'最高等級 MAX':'本級進度 '+p.current+' / '+p.target)+'</span><span>累積使用 '+p.total+' 次</span></div></article>'}
...
function historyTabs(groups){return[{id:'overview',name:'總覽'},...groups.map(g=>({id:g.id,name:g.name}))]}
function renderTabs(groups){let tabs=historyTabs(groups);return '<nav class="historyTabs" aria-label="遊戲歷程分類">'+tabs.map(t=>'<button class="historyTab'+(historyTab===t.id?' active':'')+'" data-tab="'+t.id+'">'+t.name+'</button>').join('')+'</nav>'}
function focusActiveTab(){requestAnimationFrame(()=>{let b=$('historySkills').querySelector('.historyTab.active');if(b)b.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'})})}
function setHistoryTab(id,scrollTop=false){historyTab=id;renderHistory();focusActiveTab();if(scrollTop)window.scrollTo({top:0,behavior:'smooth'})}
function shiftHistoryTab(dir){let tabs=historyTabs(catalog()),i=tabs.findIndex(t=>t.id===historyTab),next=i+dir;if(next<0||next>=tabs.length)return;setHistoryTab(tabs[next].id,false)}
function renderHistory(){let s=RPGSave.load(),st=s.stats,groups=catalog();$('historyStats').innerHTML=statsHtml(st);$('historyStats').classList.toggle('compactHidden',historyTab!=='overview');let body='';if(historyTab==='overview'){let totalUses=groups.flatMap(g=>g.items).reduce((sum,sk)=>sum+RPGSave.uses(sk.id),0),maxed=groups.flatMap(g=>g.items).filter(sk=>RPGSave.progress(RPGSave.uses(sk.id)).rank==='MAX').length;body='<section class="historyOverview"><div class="overviewCard"><span>技能累積使用</span><b>'+totalUses+'</b><small>所有可升級技能的有效觸發總次數</small></div><div class="overviewCard"><span>MAX 技能</span><b>'+maxed+'</b><small>累積使用達 90 次的技能數量</small></div></section><div class="historyHint">等級規則：0–29 次為 LV1、30–59 次為 LV2、60–89 次為 LV3、90 次以上為 MAX。技能卡只顯示目前已解鎖等級的效果；完整等級能力請查看 Type 01 Version 2 技能說明書。</div>'}else{let g=groups.find(x=>x.id===historyTab);body=g?'<section class="historyGroup paged"><div class="historyGroupTitle"><h3>'+g.name+'</h3><span>'+g.items.length+' 個技能</span></div><div class="historyGrid">'+g.items.map(skillCard).join('')+'</div></section>':''}$('historySkills').innerHTML=renderTabs(groups)+body;$('historySkills').querySelectorAll('.historyTab').forEach(b=>b.onclick=()=>setHistoryTab(b.dataset.tab,true))}
$('historySkills').addEventListener('touchstart',e=>{if(e.touches.length!==1||e.target.closest('.historyTabs')){swipeStart=null;return}let t=e.touches[0];swipeStart={x:t.clientX,y:t.clientY}},{passive:true});
$('historySkills').addEventListener('touchend',e=>{if(!swipeStart||!e.changedTouches.length)return;let t=e.changedTouches[0],dx=t.clientX-swipeStart.x,dy=t.clientY-swipeStart.y;swipeStart=null;if(Math.abs(dx)<50||Math.abs(dx)<=Math.abs(dy)*1.2)return;shiftHistoryTab(dx<0?1:-1)},{passive:true});
