...
test('version2 page imports shared core and external game',()=>{const s=read('rpg-v2-v0-2/index.html');assert(s.includes('../shared/core.js'));assert(s.includes('../shared/animations.js'));assert(s.includes('style.css'));assert(s.includes('game.js'));assert(!s.includes('<style>'))});
...
test('skill attack bursts travel from outside into target tile',()=>{const s=read('shared/animations.css');assert(s.includes('@keyframes animAttackConverge'));assert(s.includes('translate:-52px -44px'));assert(s.includes('translate:0 0'));assert(s.includes('.animBurst,.attackfx'))});
