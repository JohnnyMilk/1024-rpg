...
class GiantElephantGuard{
 ...
 async takeTurn(){if(this.state!=='ACTIVE'||!this.a.bossAlive?.())return false;const line=this.a.findChargeTarget?.();if(this.chargeCD===0&&line){await this.a.charge?.(line);this.chargeCD=1;this.tickCooldowns('charge');return true}if(this.quakeCD===0&&this.a.hasAdjacentHero?.()){await this.a.quake?.();this.quakeCD=4;this.tickCooldowns('quake');return true}await this.a.stomp?.();this.tickCooldowns();return true}
 ...
 playerPhaseAllowed(){return this.state==='WARNING'||this.state==='PLAYER_WARNING'||this.state==='DONE'}
 async afterPlayerPhase(){if(this.state!=='WARNING')return false;this.state='PLAYER_WARNING';this.emit();window.dispatchEvent(new CustomEvent('v07-collapse-player-phase'));return true}
 async enemyPhase(){if(this.state!=='PLAYER_WARNING')return false;this.state='COLLAPSING';this.emit();await this.a.collapseBottomRow?.();this.collapseRows--;if(this.collapseRows>0){this.state='WARNING';this.a.setWarningRow?.((this.a.getRows?.()||4)-1);this.a.render?.()}else{this.state='DONE';this.a.setWarningRow?.(-1);this.a.render?.()}this.emit();return true}
}
