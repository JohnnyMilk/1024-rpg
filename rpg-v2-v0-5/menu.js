...
function renderHistory(){let s=RPGSave.load(),st=s.stats,groups=catalog();$('historyStats').innerHTML=statsHtml(st);$('historyStats').classList.toggle('compactHidden',historyTab!=='overview');let body='';if(historyTab==='overview'){let totalUses=groups.flatMap(g=>g.items).reduce((sum,sk)=>sum+RPGSave.uses(sk.id),0),maxed=groups.flatMap(g=>g.items).filter(sk=>RPGSave.progress(RPGSave.uses(sk.id)).rank==='MAX').length;body='<section class="historyOverview"><div class="overviewCard"><span>技能累積使用</span><b>'+totalUses+'</b><small>所有可升級技能的有效觸發總次數</small></div><div class="overviewCard"><span>MAX 技能</span><b>'+maxed+'</b><small>累積使用達 90 次的技能數量</small></div></section><div class="historyHint">等級規則：0–29 次為 LV1、30–59 次為 LV2、60–89 次為 LV3、90 次以上為 MAX。可點擊上方分類，或在內容區左右滑動切換。</div>'}else{let g=groups.find(x=>x.id===historyTab);body=g?'<section class="historyGroup paged"><div class="historyGroupTitle"><h3>'+g.name+'</h3><span>'+g.items.length+' 個技能</span></div><div class="historyGrid">'+g.items.map(skillCard).join('')+'</div></section>':''}$('historySkills').innerHTML=renderTabs(groups)+body;$('historySkills').querySelectorAll('.historyTab').forEach(b=>b.onclick=()=>setHistoryTab(b.dataset.tab,true))}
