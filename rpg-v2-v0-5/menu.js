...
function skillCard(sk){let u=RPGSave.uses(sk.id),p=RPGSave.progress(u),pct=p.rank==='MAX'?100:Math.round(p.current/30*100);return '<article class="historySkill"><div class="hsTop"><span class="hsIcon">'+sk.icon+'</span><div><b>'+sk.name+'</b><em>'+p.rank+'</em></div></div><p class="hsDesc">'+sk.desc+'</p><div class="meter"><i style="width:'+pct+'%"></i></div><div class="hsMeta"><span>'+(p.rank==='MAX'?'MAX':'本級 '+p.current+' / 30')+'</span><span>使用 '+u+' 次</span></div></article>'}
function statsHtml(st){return [['遊戲場次',st.gamesPlayed],['總移動',st.totalMoves],['總合併',st.totalMerges],['擊殺敵人',st.totalEnemiesKilled],['擊敗 Boss',st.totalBossKills],['英雄死亡',st.totalHeroesLost],['最高英雄',st.bestHero],['最高分數',st.bestScore]].map(x=>'<div><span>'+x[0]+'</span><b>'+x[1]+'</b></div>').join('')}
...
function renderHistory(){let s=RPGSave.load(),st=s.stats,groups=catalog();$('historyStats').innerHTML=statsHtml(st);$('historyStats').classList.toggle('compactHidden',historyTab!=='overview');let body='';if(historyTab==='overview'){let totalUses=groups.flatMap(g=>g.items).reduce((sum,sk)=>sum+RPGSave.uses(sk.id),0),maxed=groups.flatMap(g=>g.items).filter(sk=>RPGSave.progress(RPGSave.uses(sk.id)).rank==='MAX').length;body='<section class="historyOverview"><div class="overviewCard"><span>技能累積使用</span><b>'+totalUses+'</b><small>所有可升級技能的有效觸發總次數</small></div><div class="overviewCard"><span>MAX 技能</span><b>'+maxed+'</b><small>累積使用達 90 次的技能數量</small></div></section><div class="historyHint">選擇上方分類查看各技能說明、等級與使用進度。</div>'}else{let g=groups.find(x=>x.id===historyTab);body=g?'<section class="historyGroup paged"><div class="historyGroupTitle"><h3>'+g.name+'</h3><span>'+g.items.length+' 個技能</span></div><div class="historyGrid">'+g.items.map(skillCard).join('')+'</div></section>':''}$('historySkills').innerHTML=renderTabs(groups)+body;$('historySkills').querySelectorAll('.historyTab').forEach(b=>b.onclick=()=>{historyTab=b.dataset.tab;renderHistory();window.scrollTo({top:0,behavior:'smooth'})})}
